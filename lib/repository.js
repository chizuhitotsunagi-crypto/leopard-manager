  ...
  if (existing) {
    await exec(
      `UPDATE daily_records SET
         record_time=?, cleaning=?, disinfection=?, maintenance=?,
    ...
    const r = await exec(
      `INSERT INTO daily_records (
         record_date, record_time, cleaning, disinfection, maintenance,
         count_status, health_status, cleaning_status,
         staff_id, notes, is_full_maintenance
       ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [p.record_date, p.record_time ?? null,
       p.cleaning ? 1 : 0, p.disinfection ? 1 : 0, p.maintenance ? 1 : 0,
       p.count_status ?? '変動なし', p.health_status ?? '良好',
       p.cleaning_status ?? '掃除済み', p.staff_id ?? null,
...
async function listRecordsByMonth(yyyymm) {
  ...
  return all(
    `SELECT d.record_date, d.record_time, d.cleaning, d.disinfection, d.maintenance,
...
module.exports = {
  listAnimals, upsertAnimal, deactivateAnimal, reorderAnimals,
  addWeight, getWeights,
  listFoods, upsertFood,
