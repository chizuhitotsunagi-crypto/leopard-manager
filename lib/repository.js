...
// その日の記録を全部削除（日次共通・給餌記録・餌の関連付け・体重）
async function deleteRecord(date) {
  const daily = await one('SELECT id FROM daily_records WHERE record_date = ?', [date]);
  if (!daily) return { deleted: false };
  // feeding_records と feeding_foods は ON DELETE CASCADE で自動削除される
  await exec('DELETE FROM daily_records WHERE id = ?', [daily.id]);
  // その日に測定した体重も消す（フルメンテ時に記録されている場合）
  await exec('DELETE FROM weights WHERE measured_at = ?', [date]);
  return { deleted: true };
}
...
module.exports = {
  ...
  getRecord, upsertRecord, deleteRecord, listRecordsByMonth, getRecentRecord, getAnimalHistory,
